...
| `StyleChartPC` | `components/charts/StyleChartPC.tsx` | ScatterChart PC版（500px・大きいDot）。YAxis domain を `dataMin-3〜dataMax+3` に自動調整。margin top=0・カードは p-2 |
| `StyleChartSP` | `components/charts/StyleChartSP.tsx` | ScatterChart SP版（aspect={1.2}・小さいDot）。YAxis domain を `dataMin-3〜dataMax+3` に自動調整。margin top=0 |
